...
/*-------------------------------------
  ...
  $(".sidebar-toggle").on("click", function () {
    ...
    if ($('#wrapper').hasClass('sidebar-collapsed')) {
      window.setTimeout(function () {
        $(".dashboard-content-one").css("margin-right", "240px");
      }, 500);
    }else{
      window.setTimeout(function () {
      $(".dashboard-content-one").css("margin-right", "8rem");
      }, 500);
    }
  });
...
document.querySelector("#img_ar_input").onchange = function() {
  const fileName = this.files[0]?.name;
  const label = document.querySelector("label[for=img_ar_input]");
  label.innerText = fileName ?? "Browse Files";
};
document.querySelector("#img_en_input").onchange = function() {
const fileName = this.files[0]?.name;
const label = document.querySelector("label[for=img_en_input]");
label.innerText = fileName ?? "Browse Files";
};